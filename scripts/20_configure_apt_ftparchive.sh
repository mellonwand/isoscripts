...
do
    wget -N http://packages.linuxdeepin.com/ubuntu/indices/override.$DIST.$i || exit 1
done
...
CONF_FILE_TEMPLATES="apt-ftparchive-deb.conf.template apt-ftparchive-extras.conf.template apt-ftparchive-udeb.conf.template release.conf.template"
...
echo Origin: Deepin >> $RELEASE_FILE
echo Label: Deepin >> $RELEASE_FILE
